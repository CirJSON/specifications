...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.375]{number}
	\caption{number}
	\label{f:nums}
\end{figure}
...
%			\newdimen\yBDot
%			\path (od);
%			\pgfgetlastxy{\xOd}{\yOd}
%			\path (bDot);
%			\pgfgetlastxy{\useless}{\yBDot}
%			\coordinate (1) at (\xOd,\yOd);
%			\coordinate (2) at (\xOd,\yBDot);
%		\end{scope}
%		\draw[-,rounded corners] (fdi.east)--(1)--(2)--(dot.west);
%	\end{tikzpicture}
%	\caption{number}
%	\label{f:nums}
%\end{figure}